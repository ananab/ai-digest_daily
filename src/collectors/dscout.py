...
class DscoutCollector(BaseCollector):
    """Dscout People Nerds 用户研究社区采集"""
    ...
    async def collect(self) -> list[CollectedItem]:
        ...
        # Try RSS feed
        rss_urls = [
            'https://dscout.com/peoplenerds/feed',
            'https://dscout.com/peoplenerds/rss',
            'https://dscout.com/blog/feed',
        ]
                ...
                if response.status_code == 200:
                    ...
                    for entry in feed.entries[:50]:
                        ...
                        items.append(CollectedItem(
                            ...
                            summary=summary,
                            url=link,
                            source='Dscout People Nerds',
                            category='news',
                            published_date=_parse_date(entry),
                        ))
                    logger.info(f'Dscout RSS: 采集到 {len(items)} 条新闻')
                    return items
            except Exception as e:
                logger.debug(f'Dscout RSS {rss_url} 失败: {e}')

        # Fallback: HTML scraping
        try:
            response = await self.client.get('https://dscout.com/peoplenerds')
            ...
            logger.info(f'Dscout HTML: 采集到 {len(items)} 条新闻')
        except Exception as e:
            logger.error(f'Dscout 采集失败: {e}')
