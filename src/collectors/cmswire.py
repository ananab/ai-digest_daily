...
class CMSWireCollector(BaseCollector):
    """CMSWire 内容管理/CX新闻采集"""
        ...
        for rss_url in rss_urls:
            try:
                ...
                if response.status_code == 200:
                    ...
                    logger.info(f'CMSWire RSS: 采集到 {len(items)} 条新闻')
                    return items
            except Exception as e:
                logger.debug(f'CMSWire RSS {rss_url} 失败: {e}')

        # Fallback: HTML scraping
        try:
            response = await self.client.get('https://www.cmswire.com/')
            ...
            logger.info(f'CMSWire HTML: 采集到 {len(items)} 条新闻')
        except Exception as e:
            logger.error(f'CMSWire 采集失败: {e}')
